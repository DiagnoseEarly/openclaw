...
# SKILL_API_KEY is the standard for OpenClaw skills
# PIPEDRIVE_API_TOKEN is for backward compatibility or direct use
if [ -n "$SKILL_API_KEY" ]; then
  PIPEDRIVE_API_TOKEN="$SKILL_API_KEY"
elif [ -n "$PIPEDRIVE_API_TOKEN" ]; then
  : # It's already set
else
  PIPEDRIVE_API_TOKEN=""
fi
...
if [ -z "$PIPEDRIVE_API_TOKEN" ]; then
  echo "Error: API token not found."
  echo "Please configure the skill in openclawd.json with the 'apiKey' property."
  echo "Alternatively, set the SKILL_API_KEY or PIPEDRIVE_API_TOKEN environment variable."
  exit 1
fi
